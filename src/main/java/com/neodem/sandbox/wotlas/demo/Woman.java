...
import com.neodem.graphics.dd.engine.wotlas.drawable.AuraEffect;
import com.neodem.graphics.dd.engine.wotlas.drawable.ShadowSprite;
...
import com.neodem.graphics.dd.engine.wotlas.drawable.TextDrawable;
   ...
   */
   ...
   // 4 - Finally, we add Sprite & Shadow to the GraphicsDirector
      if(!isPlayerControlled) {
          ...
          gDirector.addDrawable(shadow); // Our Woman sprite has already been added to our GraphicsDirector
   ...
   */
    public void tick() {
     ...
     // 2 - Player out of the limit of the screen ?
        if( x<0 ) x = (float) (backgroundDim.width-5);
        else if( x>=backgroundDim.width ) x = 5.0f;

        if( y<0 ) y = (float) (backgroundDim.height-5);
        else if( y>=backgroundDim.height ) y = 5.0f;
   ...
   public Drawable getTextDrawable() {
       if( textDrawable!=null ) {
           if( textDrawable.isLive() )
               return null;  // text already displayed on screen
